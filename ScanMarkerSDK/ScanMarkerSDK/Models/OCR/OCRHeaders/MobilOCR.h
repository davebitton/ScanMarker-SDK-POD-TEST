...
typedef unsigned int TFineImageProcessingOptions;
...
typedef enum tagTFineImageProcessingOptionsFlags {
	...
	/* Don't use fast binarization. It works slower but can give better quality on CJK text */
...
typedef enum tagTFineTextBlockAttributes {
	...
	/* Text block inversion is unknown - it needs to be detected. Valid only for FineRecognizeBlocks call. */
	FTBA_AutodetectInversion = BIT_FLAG( 2 ),

    /* Dummy text  block - just a picture. */
	FTBA_PictureBlock = BIT_FLAG( 0x07 )
 ...
 *    type of warning. */
typedef int (*TFineProgressCallbackFunction)( int processedPercentage, unsigned int warning, void* warningData );
...
/* Sets the number of threads using during recognition. By default equals 4.
 * threadsCount [in] is a maximum allowed number of threads. Value must be posititve.
 */
FINE_API_LINKAGE
TFineErrorCode FineSetRecognizerThreadsCount(
    int threadsCount
    );
...
TFineErrorCode FineRecognizeBarcode(
    ...
    unsigned int allowedSupplements,
    int hasChecksum,
...
/* Extract barcodes from given image.
 * image [in] is an image to be processed
 * allowedTypes [in] is a set of barcode types
 * allowedOrientations [in] is a set of barcode orientations
 * allowedSupplements [in] is a set of barcode supplements.
 ...
 *    in current codepage. Otherwise it will be put out in hex form.
 * defaultCodePage [in] is a codepage for barcodes which mismatch with specification.
 * layoutBuff [out] is a recognition result. Must be released through FineFreeMemory.
 * progressCallback [in] is a callback function for progress information. Can be zero
 */
FINE_API_LINKAGE
...
/* Performs the image document analysis, finds blocks, and returns them. No recognition is performed.
...
/* As FineRecognizeRegion, but no analysis will be called on region provided,
* the lines will be reextracted and rerecognized in this method
...
* dictionaries [in] is a zero-terminated list of dictionaries
* image [in] is an image to be processed
* blockCount [in] the number of blocks
* blocks [in] the blocks to be recognized
*    CFineTextBlock structure needs to have only RegionRects, RegionRectsCount and Attributes members filled
* imageProcessingOptions [in] is image processing options
* recMode [in] is a recognition mode
* confidenceLevel [in] is a recognition confidence level
* layoutBuff [out] is tht recognition result.
