...
CHebrewCompressChina_1::CHebrewCompressChina_1(void)
{
	m_pWorkData = NULL;
	m_pWorkData1 = NULL;
	m_pWorkData2 = NULL;
 ...
 CHebrewCompressChina_1::~CHebrewCompressChina_1()
{
		...
		if (m_chinaItalic)
		{
			delete m_chinaItalic;
			m_chinaItalic = NULL;
		}

 
}
void CHebrewCompressChina_1::WriteDebugStr(char * str, bool isAppend)
{
return;
	FILE * fff = NULL;
	if (isAppend)
		fff = fopen("C://Temp//CHebrewCompressChina_1Debug.txt", "at");
	else
		fff = fopen("C://Temp//CHebrewCompressChina_1Debug.txt", "wt");
...
void CHebrewCompressChina_1::AllocateMemory()
{
		//m_width += 600;
		//m_output_width += 600;
		...
		m_pWorkData3 = new float[m_output_width];
		...
		m_pWorkData2 = new float[m_output_width*2];;
		...
		m_pWorkData4 = new int[m_output_width*m_height];;
		...
		m_pWorkData5 = new int[m_output_width];;
		...
		m_chinaItalic = new ChineseItalic();

}

void CHebrewCompressChina_1::Init(bool corierActive,int width, int height, int outputWidth, byte* pDataIn, byte* pDataOut, byte* pDataIn3)
{
	m_width = width;
...
void CHebrewCompressChina_1::ReadCfgFile(char* dir)
{
	return;
	char fileName[255];
...
void CHebrewCompressChina_1::CalculateWidthLines()
{
	...
	//int range = 300;
	for ( int i = m_begin_x; i < m_end_x-1; i+=40)
	{


		int iLeft = m_end_x;
		int iRight = m_begin_x;

		FindLimitsByChanges(1000, i, iLeft, iRight);
...
/*
	//for ( int i = m_begin_x+range; i < m_end_x-range-1; i+=40)
	{
...

		{
			...
			for ( int ii = i1; ii < i2; ii++)
			{
				m_pWorkData3[ii] = widthVerticalResult;
			}
...
void CHebrewCompressChina_1::CalculateWidthLinesKorean()
{
	for ( int i = 0; i < m_width; i++)
	{
		m_pWorkData2[i] = -1;
...
void CHebrewCompressChina_1::FixByWidthWordKorean()
{
	...
	for ( int l = 0; l < m_numberLetters; l++)
	{
		...
		i1+=2;
		while (!m_isEmptyColumn[i1] && i1 > 0) i1--;
		i1++;
		i2-=2;
		while (!m_isEmptyColumn[i2] && i2 < m_width-1) i2++;
		//i2--;
...
void CHebrewCompressChina_1::CalculateWidthPerChange(int window, int step)
{
...
void CHebrewCompressChina_1::CalculateImageBoundaries()
{
	m_begin_x = m_width-1;
	m_end_x = 0;
	m_begin_y = 0;
...
/*
void CHebrewCompressChina_1::FixInterpolationOutput(int i1, int i2, int j1, int j2)
{
	if ( i2 - i1 < 7) return;
	if ( j2 - j1 < 20) return;
...
int CHebrewCompressChina_1::Calculate(void *job, int* d, int* o)
{
	...
	m_chinaItalic->Init(m_width, m_height, m_pDataIn);
	m_chinaItalic->Calculate();
	CalculateLetterEdgesChinese();
	CalculateWidthLines();
	
	if (m_numberLetters <= 2) return -1000; 
	CalculateWidthPerChange(300,20);

	CalculateLettersRelativeWidth();
	
	float medianHeight = CalculateMedianHeight();
	...
	for (int l = 0; l < m_numberLetters; l++)
	{
		...
		{
            if ( i < 0 || i>= m_width-1) continue;
			m_pInterpolationOutput[i] = m_widthPerChange[i] = m_letterRelativeWidth[l]*medianHeight/(float)(m_letterEdges[l][2]-m_letterEdges[l][0]);
		}
...
void  CHebrewCompressChina_1::FindLimitsByChanges(int numberChangesMax, int iMiddle, int &iLeft, int &iRight)
{
	iLeft = m_end_x;
	iRight = m_begin_x;

	memset(m_pWorkData4, 0, m_output_width*m_height*sizeof(int));
	memset(m_pWorkData5, 0, m_output_width*sizeof(int));
...
float CHebrewCompressChina_1::CalculateMedianHeight()
{

	float sortedHeight[_MAX_NUMBER_LETTERS_];
	for (int l = 0; l < m_numberLetters;l++)
	{
		sortedHeight[l] = m_letterEdges[l][3] - m_letterEdges[l][1];
	}
	...
	for (int l = 0; l < m_numberLetters-1; l++)
	{
		for (int l1 = l+1; l1 < m_numberLetters; l1++)
		{
			if (sortedHeight[l] < sortedHeight[l1])
			{
				float tmp = sortedHeight[l];
				sortedHeight[l] = sortedHeight[l1];
				sortedHeight[l1] = tmp;
			}
		}
	}

	float medianHeight = 52.0;//sortedHeight[m_numberLetters/2] * 1.1;

	return medianHeight;
}
	
void CHebrewCompressChina_1::CalculateLettersRelativeWidth()
{
	
	float ratioVerticalToHorizontalLines[_MAX_NUMBER_LETTERS_];


	for (int l = 0; l < m_numberLetters; l++)
	{
		ratioVerticalToHorizontalLines[l] = 0;
		m_letterRelativeWidth[l] = 0;
	}
	...
	for ( int l = 0; l < m_numberLetters; l++)
	{
		...
		for ( int i = m_letterEdges[l][0]-20 ; i < m_letterEdges[l][2]+20; i++)
		{
			if (i < 0) continue;
			if (i >= m_width) break;
		...
		float widthVerticalLines = 0;
		int numberVerticalLines = 0;
	
		
		for ( int i = m_letterEdges[l][0] ; i < m_letterEdges[l][2]; i++)
		{
			if (m_pWorkData3[i] > 0.0001)
			{
				widthVerticalLines += m_pWorkData3[i];
				numberVerticalLines++;
			}
		}
		int widthLetter = m_letterEdges[l][2] - m_letterEdges[l][0];
		int heightLetter = m_letterEdges[l][3] - m_letterEdges[l][1];
		if ( numberLines > 25 || numberLines >= (widthLetter / 3))
		{
			float aveWidthLines = widthLines / (float) numberLines;
			m_unitsInLetter[l] = (float) (m_letterEdges[l][2] - m_letterEdges[l][0]) / aveWidthLines;

			if (numberVerticalLines > 25 || numberVerticalLines > heightLetter/3)
			{
				float aveWidthVerticalLines = widthVerticalLines / (float) numberVerticalLines;
				ratioVerticalToHorizontalLines[l] = aveWidthVerticalLines / aveWidthLines;
			}
			else
			{
				ratioVerticalToHorizontalLines[l] = 0.0;
			}
		}
		else
		{
			m_unitsInLetter[l] = 0.0;
			ratioVerticalToHorizontalLines[l] = 0.0;
		}
	...
	float sortedUnitsInLetter[_MAX_NUMBER_LETTERS_];

	for (int l = 0; l < m_numberLetters; l++)
	{
		sortedUnitsInLetter[l] =  m_unitsInLetter[l];
	}
	...
	for (int l = 0; l < m_numberLetters-1; l++)
	{
		for (int l1 = l+1; l1 < m_numberLetters; l1++)
		{
			if (sortedUnitsInLetter[l] < sortedUnitsInLetter[l1])
			{
				float tmp = sortedUnitsInLetter[l];
				sortedUnitsInLetter[l] = sortedUnitsInLetter[l1];
				sortedUnitsInLetter[l1] = tmp;
			}
		}
	}
	int nL = m_numberLetters;
	for (int l = 0; l < m_numberLetters; l++)
	{
		if (sortedUnitsInLetter[l] < 0.001)
		{
			nL = l;
			break;
		}
	...
	float unitsInRelativeWidth_1_0 = 0.0;
	/*if (m_numberLetters > 20)
	{
		unitsInRelativeWidth_1_0 = sortedUnitsInLetter[m_numberLetters/5];
	}
	else if (m_numberLetters > 10)
	{
		unitsInRelativeWidth_1_0 = sortedUnitsInLetter[m_numberLetters/3];
	}
	else if (m_numberLetters > 5)
	{
		unitsInRelativeWidth_1_0 = sortedUnitsInLetter[m_numberLetters/2];
	}
	else if (m_numberLetters > 3)
	{
		unitsInRelativeWidth_1_0 = sortedUnitsInLetter[m_numberLetters-2];
	}
	else
	{
			unitsInRelativeWidth_1_0 = sortedUnitsInLetter[0];
	}*/
	unitsInRelativeWidth_1_0 = sortedUnitsInLetter[nL/2];//m_numberLetters/2];
	/*
	float minNearestTo1_0 = 1000;
	int l_NearestTo1_0 = -1;
	for ( int l = 0; l < m_numberLetters; l++)
	{
		int widthLetter = m_letterEdges[l][2] - m_letterEdges[l][0];
		int heightLetter = m_letterEdges[l][3] - m_letterEdges[l][1];
		if (heightLetter < 49) continue;
		float ratioHeightToWidth = (float)heightLetter/ (float)widthLetter; 
		float ratio = ratioVerticalToHorizontalLines[l] / ratioHeightToWidth;
		if (fabs(ratio-1.0) < fabs(minNearestTo1_0-1.0))
		{
			minNearestTo1_0 = ratio;
			l_NearestTo1_0 = l;
		}
	}

	float unitsInRelativeWidth_1_0 = m_unitsInLetter[l_NearestTo1_0];
	*/
	for (int l = 0; l < m_numberLetters; l++)
	{
		float ratio  = m_unitsInLetter[l]/unitsInRelativeWidth_1_0;

	/*	if (ratio > 1.5)
			m_letterRelativeWidth[l] = ratio;
		/*else if (ratio > 3.5)
			m_letterRelativeWidth[l] = 4.0;
		else if (ratio > 2.5)
			m_letterRelativeWidth[l] = 3.0;
		else if (ratio > 1.7)
			m_letterRelativeWidth[l] = 2.0;
		* /
		else if (ratio > 0.8)
			m_letterRelativeWidth[l] = 1.0;
		else if (ratio > 0.5)
			m_letterRelativeWidth[l] = 0.7;
		else
			m_letterRelativeWidth[l] = ratio; 
	*/
...
void CHebrewCompressChina_1::FixWidthByColorChange()
{
...
void CHebrewCompressChina_1::CalculateLetterEdgesChinese()
{
	m_numberLetters = m_chinaItalic->m_numberLetters;
	
	
	for ( int l = 0; l < m_numberLetters; l++)
	{
		m_letterEdges[l][0] = m_chinaItalic->m_beginLetters[l];
		m_letterEdges[l][2] = m_chinaItalic->m_endLetters[l];

		int i_begin = m_letterEdges[l][0]-20;
		int i_end = m_letterEdges[l][2]+20;

		if (i_begin < 0) i_begin = 0;
		if (i_end >= m_width) i_end = m_width-1;

		int j_begin = m_height-1;
		int j_end = 0;
		...
		if (j_end >= m_height) j_end = m_height-1;

		m_letterEdges[l][1] = j_begin;
		m_letterEdges[l][3] = j_end;
	}

}

void CHebrewCompressChina_1::CalculateLetterEdgesVerticalAsia()
{
	m_numberLetters = 0;
...
void CHebrewCompressChina_1::CalculateCompressRatio()
{
	...
	for ( int l = 0; l < m_numberLetters; l++)
	{
		...
		if ( numberLines > 25 || numberLines >= (widthLetter / 3))
		{
			float aveWidthLines = widthLines / (float) numberLines;
			m_unitsInLetter[l] = (float) (m_letterEdges[l][2] - m_letterEdges[l][0]) / aveWidthLines;
			ratioHeightsToUnitsWidth[l] = (float) (m_letterEdges[l][3] - m_letterEdges[l][1]) / m_unitsInLetter[l];
			if (numberVerticalLines > 25 || numberVerticalLines > heightLetter/3)
			{
				float aveWidthVerticalLines = widthVerticalLines / (float) numberVerticalLines;
				ratioVerticalToHorizontalLines[l] = aveWidthVerticalLines / aveWidthLines;
	...
	*/
	...
	while (heightMiddleLetter < 25 || numberChanges[indexMiddleLetter] < 3*heightMiddleLetter - 30 || widthMiddleLetter > 500 ||
		 m_unitsInLetter[indexMiddleLetter] > 30.0 || ((indexMiddleLetter < 2 || indexMiddleLetter > m_numberLetters - 2) && m_numberLetters > 4) ||
		 (m_unitsInLetter[indexMiddleLetter] < 5 ))
	{
	...
	float numberUnitsInMiddleLetter = m_unitsInLetter[indexMiddleLetter];
	...
	for ( int l = 0; l < m_numberLetters; l++)
	{
		...
		float desiredWidth = usedWidthUnit * m_unitsInLetter[l];
...
void CHebrewCompressChina_1::Compress(int isTempOutput, int flag)
{
	...
	int i;
	for ( i = m_begin_x; i <= m_begin_x + numberInAverage; i++)
	{
		if (i < 0) continue;
	...
	if (count)
	{
		for ( int ii = m_begin_x; ii <= i; ii++)
		{
			if (ii < 0) continue;
			m_pInterpolationOutput[ii] = ave / count;
		}
	}
	for ( i = m_begin_x+halfOfAve; i < m_end_x; i++)
	{
	...
	for ( int i = m_end_x; m_end_x+halfOfAve; i++)
	{
		if (i >= m_width) break;
		m_widthPerChange[i] = m_pInterpolationOutput[i] = lastValue;
	}
	...
	for ( int i = m_begin_x; i <= m_end_x; i++)
	{
		...
		int end_x = m_end_x+halfOfAve;;
		if (end_x >= m_width) end_x = m_width-1;
		for ( int k = 0; k < add; k++)
		{
			for ( int j = 0; j < m_height; j++)
			{
				...
				{
					if ( i < m_begin_x  || i > end_x ) continue;
				...
				{
					if ( i < m_begin_x || i > m_end_x ) continue;
...
#ifdef _WRITE_DEBUG_BMP_
	FILE* fp = fopen("C://Temp//debug01bmp", "wb");
	if (fp)
	{
...
void CHebrewCompressChina_1::DrawOutput(void* values, int valuesType, int red, int green, int blue, float minValue, float maxValue)
{
