...
static int   loadBMP32  PARM((FILE *, byte *, u_int, u_int));
...
static void  writeBMP32 PARM((FILE *, byte *, int, int));
...
/*******************************************/
{
  ...
  fp = xv_fopen(fname,(char *)"rb");
  if (!fp) return (bmpError(bname, (char *)"couldn't open file"));
...
//    goto ERROR;
  ...
  if (biBitCount==24) {
    ...
    if (!pic24) return (bmpError(bname, (char *)"couldn't malloc 'pic24'"));
  }
    ...
    if (!pic8) return(bmpError(bname, (char *)"couldn't malloc 'pic8'"));
  }
...
//  WaitCursor();
  ...
  if      (biBitCount == 1) rv = loadBMP1(fp,pic8,biWidth,biHeight);
					  ...
					  biCompression);
							...
							rv = loadBMP24(fp,pic24,biWidth,biHeight);
 else  if (biBitCount==32)
							rv = loadBMP32(fp,pic24,biWidth,biHeight);

  if (rv) bmpError(bname, (char *)"File appears truncated.  Winging it.\n");
...
/*******************************************/
static int loadBMP1(FILE *fp, byte *pic8, u_int  w, u_int  h)
...
/*******************************************/
static int loadBMP4(FILE *fp, byte *pic8, u_int  w, u_int  h, u_int  comp)
...
/*******************************************/
static int loadBMP8(FILE *fp, byte *pic8, u_int  w, u_int  h, u_int  comp)
...
/*******************************************/
static int loadBMP24(FILE *fp, byte *pic24, u_int  w, u_int  h)
{
  int   i,j,padb,rv;
  byte *pp;
...
static int loadBMP32(FILE *fp, byte *pic24, u_int w, u_int h)
{
  int   i,j,padb,rv;
  byte *pp;
...
//    if (!graypic) FatalError("unable to malloc in WriteBMP()");
  ...
  else if (ptype == PIC32) {  /* is F_FULLCOLOR */
    nbits = 32;
    cmaplen = 0;
    nc = 0;
  }
  ...
  else if (nbits == 32) writeBMP32(fp, pic824, w, h);
...
/*******************************************/
static void writeBMP32(FILE* fp, byte* pic24, int w, int h)
{
  int   i,j,c,padb;
  byte *pp;
